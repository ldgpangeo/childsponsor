...
if ($row['is_active'] <> 'N') {
    throw NEW Exception("This recon is already active.", ERROR_MINOR);
}
...
if ($_POST['submit'] <> '') {
    ...
    $sql = "update r_recon set is_active = 'Y', status = '{$in['status']}' where reconid = '$reconid' limit 1";
    $res = do_sql($sql);
    $redirect = "index.php";
}
...
<h2 align="center">Caution!  You are about to resume a terminated sponsorship reconciliation.</h2>

<h3>Reason for this resumption: <input type ="text" size="80" name="status"> </h3>
<p align="center">
